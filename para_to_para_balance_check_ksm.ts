...
async function main () {
    const provider = new WsProvider('wss://basilisk-rpc.n.dwellir.com');
    const api = await ApiPromise.create({ provider });
  
    //Check dot balance
    let data: any = await api.query.tokens.accounts(Account, 14);
    let previousFree = data.free;
    console.log(`${Account} has a balance of ${data.free}`);
  
    // Here we subscribe to any balance changes and update the on-screen value

    api.query.tokens.accounts(Account, 14, ({ free: currentFree }: { free: any }) => {
      // Calculate the delta
